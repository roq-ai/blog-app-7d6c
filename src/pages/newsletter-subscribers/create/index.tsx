...
import { createNewsletterSubscriber } from 'apiSdk/newsletter-subscribers';
...
function NewsletterSubscriberCreatePage() {
  ...
  const handleSubmit = async (values: NewsletterSubscriberInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createNewsletterSubscriber(values);
      resetForm();
...
)(NewsletterSubscriberCreatePage);
